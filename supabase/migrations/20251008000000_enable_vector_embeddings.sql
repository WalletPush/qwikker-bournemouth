-- Migration: Enable Vector Embeddings for AI Knowledge Base
-- Description: Adds pgvector extension and embedding column to existing knowledge_base table
-- Date: 2025-10-08 00:00:00 UTC

-- Enable the pgvector extension for vector similarity search
CREATE EXTENSION IF NOT EXISTS vector;

-- Add embedding column to existing knowledge_base table
ALTER TABLE public.knowledge_base 
ADD COLUMN IF NOT EXISTS embedding vector(1536); -- OpenAI ada-002 embeddings are 1536 dimensions

-- Add vector similarity index to knowledge_base
CREATE INDEX IF NOT EXISTS idx_knowledge_base_embedding 
ON public.knowledge_base USING HNSW (embedding vector_cosine_ops) 
WHERE embedding IS NOT NULL;

-- Create function for vector similarity search on knowledge_base
