-- Migration: Create multiple offers system
-- Description: Creates a separate business_offers table to support multiple offers per business
-- based on their subscription tier (Starter: 1, Featured: 3, Spotlight: unlimited)
-- Date: 2025-09-29 20:00:00 UTC
...
-- Create indexes
CREATE INDEX idx_business_offers_business_id ON public.business_offers(business_id);
CREATE INDEX idx_business_offers_status ON public.business_offers(status);
CREATE INDEX idx_business_offers_approved_at ON public.business_offers(approved_at DESC);
CREATE INDEX idx_business_offers_display_order ON public.business_offers(business_id, display_order);
...
-- Migrate existing offers from business_profiles to business_offers
