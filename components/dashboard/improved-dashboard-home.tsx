  ...
  useEffect(() => {
    if (profile?.user_id) {
      const loadActivityFeed = async () => {
        ...
        // 6. NOTIFICATIONS-DRIVEN ACTIVITY (loyalty joins, stamp earns, redemptions, saves)
        try {
          const res = await fetch('/api/business/notifications?limit=20')
          if (res.ok) {
            const { notifications } = await res.json()
            const NOTIF_TYPE_MAP: Record<string, { icon: React.ReactNode; color: string }> = {
              loyalty_join: {
            ...
            const feedTypes = new Set(Object.keys(NOTIF_TYPE_MAP))
            for (const n of (notifications || [])) {
              if (!feedTypes.has(n.type)) continue
          ...
          // Non-critical — feed still works from other sources
        }
        ...
        <Card className="bg-slate-800/50 border-slate-700 relative overflow-hidden">
          <CardHeader>
            <CardTitle className="text-white flex items-center gap-2">
              ...
              Social Wizard
              <span className="text-xs font-normal text-slate-500 bg-slate-700/50 px-2 py-0.5 rounded-full">Coming Soon</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              <p className="text-slate-400 text-sm">
                AI-powered social content generation for Instagram, Facebook, and more. Create branded posts in seconds.
              </p>
              <div className="bg-slate-700/30 rounded-lg p-3 border border-slate-600/30">
                ...
                <ul className="text-slate-400 text-xs mt-2 space-y-1">
                  <li>Brand-matched post templates</li>
                  <li>AI caption and hashtag generation</li>
