...
export const BOTTOM_TABS: BottomTabItem[] = [
  {
    ...
    label: 'Nearby',
    href: '/user/dashboard',
    matchSections: ['dashboard', 'discover'],
    icon: (
  ...
  },
  {
    ...
    label: 'More',
    href: '/user/more',
    matchSections: ['more', 'settings', 'notifications', 'events', 'secret-menu', 'badges', 'how-it-works'],
    icon: (
      <svg className={iconClass} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
...
export function UserBottomNav({
  ...
  return (
    <nav
      className="lg:hidden fixed bottom-0 inset-x-0 z-[90] border-t border-zinc-800/80 bg-black/95 backdrop-blur-xl"
        ...
        {BOTTOM_TABS.map((tab) => {
          ...
          return (
            <PendingLink
              ...
              className={`relative flex flex-1 flex-col items-center justify-center gap-0.5 py-1 min-h-[48px] touch-manipulation transition-colors ${
                isActive ? 'text-[#00d083]' : 'text-zinc-500 active:text-zinc-300'
              }`}
            >
