  ...
  const fetchComprehensiveAnalytics = async () => {
    ...
    try {
      ...
      const comprehensiveAnalytics: ComprehensiveAnalytics = {
        ...
        realQRData,
        totalPushNotifications: additionalData.totalPushNotifications || 0,
        pushesThisWeek: additionalData.pushesThisWeek || 0,
        totalPushRecipients: additionalData.totalPushRecipients || 0,
        totalPushClicks: additionalData.totalPushClicks || 0
      }
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-slate-700/30 rounded-lg p-4">
                  <div className="text-2xl font-bold text-white">{analytics.totalPushNotifications || 0}</div>
                  <div className="text-slate-400 text-sm">Total Sent</div>
                </div>
                <div className="bg-slate-700/30 rounded-lg p-4">
                  <div className="text-2xl font-bold text-blue-400">{analytics.pushesThisWeek || 0}</div>
                  <div className="text-slate-400 text-sm">This Week</div>
                </div>
                <div className="bg-slate-700/30 rounded-lg p-4">
                  <div className="text-2xl font-bold text-green-400">{analytics.totalPushRecipients || 0}</div>
                  <div className="text-slate-400 text-sm">Recipients</div>
                </div>
                <div className="bg-slate-700/30 rounded-lg p-4">
                  <div className="text-2xl font-bold text-purple-400">
                    {analytics.totalPushRecipients && analytics.totalPushRecipients > 0 
                      ? ((analytics.totalPushClicks! / analytics.totalPushRecipients) * 100).toFixed(1) 
                      : '0.0'}%
                  </div>
