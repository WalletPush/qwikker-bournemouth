...
export function AdminDashboard({ businesses, adminEmail }: AdminDashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<'pending' | 'live' | 'incomplete' | 'changes'>(() => {
    const urlTab = searchParams.get('tab')
    const validTabs = ['pending', 'live', 'incomplete', 'changes']
    return validTabs.includes(urlTab || '') ? (urlTab as any) : 'pending'
  })
  const [sidebarOpen, setSidebarOpen] = useState(false)
  
  // Function to update tab and URL
  const updateActiveTab = (newTab: 'pending' | 'live' | 'incomplete' | 'changes') => {
    setActiveTab(newTab)
  ...
  const pendingBusinesses = businessList.filter(b => b.status === 'pending_review' || b.status === 'pending')
            ...
            <div className="space-y-6">
              {incompleteBusinesses.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-300 mb-2">No incomplete listings</h3>
