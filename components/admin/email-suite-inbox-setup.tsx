...
export function EmailSuiteInboxSetup({ city }: EmailSuiteInboxSetupProps) {
  ...
  const mxHost = 'inbound-smtp.eu-west-1.amazonaws.com'
    ...
    >
      ...
      <div className="px-4 pb-4 space-y-5 border-t border-slate-700/80 pt-4 text-sm">
        <div className="rounded-lg border border-slate-600/80 bg-slate-900/50 p-3 space-y-3">
          ...
          <CopyField label="Reply-to / test address" value={replyTo} />
          <CopyField label="Cloudflare MX name" value={citySlug} />
          <CopyField label="Cloudflare mail server (hostname only)" value={mxHost} />
        </div>
        ...
        <ol className="space-y-4 list-decimal list-outside pl-5 text-slate-300">
          ...
          <li className="space-y-1">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              ...
              and switch it on. Resend shows the MX record to add. Keep a note of the mail server
              hostname (usually <span className="font-mono text-slate-300">{mxHost}</span> for EU).
            </p>
          </li>
          ...
          <li className="space-y-1">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              DNS for <span className="font-mono text-slate-300">qwikker.com</span> → Add record:
            </p>
            <ul className="text-xs text-slate-400 space-y-1 list-disc pl-4">
              <li>
                Type: <span className="text-slate-300">MX</span>
              </li>
              <li>
                Name: <span className="font-mono text-slate-300">{citySlug}</span> only — not the full
                domain, not <span className="font-mono">@</span>
              </li>
              <li>
                Mail server: <span className="font-mono text-slate-300">{mxHost}</span>
              </li>
              <li>
                Priority: <span className="text-slate-300">10</span> (or whatever Resend shows)
              </li>
              <li>TTL: Auto</li>
            </ul>
            <p className="text-xs text-amber-200/90 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2 mt-2 leading-relaxed">
              Mail server must be a hostname only. Do not paste{' '}
              <span className="font-mono">http://…</span> or a trailing slash — Cloudflare will reject
              it. There should be only one MX on <span className="font-mono">{citySlug}</span> (the{' '}
              <span className="font-mono">send.{citySlug}</span> MX for sending is separate and should
              stay).
            </p>
          </li>
          ...
          <li className="space-y-1">
            ...
            <ul className="text-xs text-slate-400 space-y-1 list-disc pl-4">
              ...
              <li>
                ...
                <span className="font-mono text-slate-300">email.failed</span>, and optionally{' '}
                <span className="font-mono text-slate-300">email.opened</span> /{' '}
                <span className="font-mono text-slate-300">email.clicked</span>
              </li>
            </ul>
        ...
        <div className="text-xs text-slate-500 leading-relaxed border-t border-slate-700/80 pt-3">
          HQ usually owns the Cloudflare zone for qwikker.com. Franchise partners who only have a
          Resend login still need someone with DNS access for the MX step. One webhook URL per city
          subdomain.
        </div>
      </div>
    </details>
