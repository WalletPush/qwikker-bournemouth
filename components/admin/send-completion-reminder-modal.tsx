...
interface SendCompletionReminderModalProps {
  isOpen: boolean
  onClose: () => void
...
export function SendCompletionReminderModal({
  isOpen,
  onClose,
  business,
  onSent,
  onError,
}: SendCompletionReminderModalProps) {
  ...
  const loadPreview = useCallback(async () => {
    ...
    setPreview(null)
    try {
      const res = await fetch('/api/admin/send-completion-reminder', {
        method: 'POST',
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok || !data.success) {
        onError(data.error || 'Failed to send reminder')
        setIsSending(false)
        return
      }
      ...
      <DialogContent className="sm:max-w-[640px] bg-slate-900 border-slate-700 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            ...
            <DialogTitle className="text-xl text-white">Send Completion Reminder</DialogTitle>
          </div>
          <DialogDescription className="text-slate-300 text-base">
            Review the email below, then confirm to send it from your city&apos;s QWIKKER address.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {loadError && !isLoadingPreview && (
            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg flex items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-red-400">Can&apos;t send a reminder</div>
          ...
          {preview && !isLoadingPreview && (
            <>
              ...
              <div className="grid grid-cols-1 gap-2 text-sm">
                ...
                <div className="flex gap-2">
                  <span className="text-slate-400 w-16 flex-shrink-0">Missing</span>
                  <span className="text-orange-300 font-medium">
                    {preview.missingItems.length} item{preview.missingItems.length === 1 ? '' : 's'} · {preview.completionPercentage}% complete
                  </span>
                </div>
              </div>
