...
interface AdminDashboardProps {
  ...
  pendingLoyaltyCount?: number
}

export function AdminDashboard({ businesses, crmData, adminEmail, city, cityDisplayName, pendingChangesCount, pendingChanges, pendingMenus, pendingEvents, walletPassesCount, pendingLoyaltyCount = 0 }: AdminDashboardProps) {
                  ...
                  {item.count !== undefined && (
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      activeTab === item.id
                        ...
                        : item.count > 0 && (item.id === 'incomplete' || item.id === 'pending' || item.id === 'updates' || item.id === 'claims' || item.id === 'contact-centre' || item.id === 'loyalty-queue')
...
Qwikker Admin Team`
              ...
              {activeTab === 'knowledge' && (
                <KnowledgeBaseTab city={city} cityDisplayName={cityDisplayName} adminId={adminEmail} />
              )}
              ...
              {/* Loyalty Queue Tab */}
              {activeTab === 'loyalty-queue' && (
                <AdminLoyaltyQueue city={city} />
              )}
