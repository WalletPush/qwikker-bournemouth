...
**Related:** admin CRM offer edit (`admin_offer_edit_e23864ab`) · media library + framing · roadmap `dennis-08` / `dennis-15` Layer 1 **completed**.

### Shipped on `main`
- **Schema:** `user_saved_offers`, `offer_activations`, `activate_offer` RPC, `activation_window_minutes` (30/60/120). Migrations applied manually in Supabase.
- **Consumer:** Save → Redeem now → replace-active confirm; Active filter + live `m:ss` countdown; hydrate from `GET /api/offers/activate`.
- **Offers UI (Aug 10):** primary All / Saved / Active; sticky type + category chips (% Off, 2-for-1, Ending soon, Favourites). Commit `18501087`.
- **Wallet:** silent `Current_Offer` then one `Last_Message` push (avoids double “Store Card Changed”); outbox/retry; clear skips wipe if another offer already active.
- **Cron** `/api/cron/offer-activations` every 10m: mid-window warning once; expiry → clear front + personalised vibe CTA push (`How was your {offer}, {name}?…` + `?vibe=1` deep link). Mid-window verified live; `CRON_SECRET` set (Bournemouth).
- **Merchant / admin:** Wallet duration select; CRM immediate offer / featured / secret-menu edit + offer images (even after claim).
- **Analytics:** Saved / Activated / Activation ended (prefer activations tables).
- **Ops:** Multi-tenant cron, per-city WalletPush keys.

### Still open (not R1 blockers)
| Item | Notes |
|---|---|
| Chat Save / Redeem wording | Still Claim / Add to Wallet in places |
| R2 QR install attribution (Jerry taxi/partner cents) | Plan pending — roadmap must-do with media leftovers |
| Business print-pack PDF download | SVG artwork exists; export “Coming soon” |
...
## ✅ Media library + admin image curation — SHIPPED / HARDENED (Aug 9–10, 2026)

**What shipped:** `media_assets` catalogue, hero/offer pointers, framing editor (Fill / Show whole, drag, zoom), claimed + unclaimed curation, pending owner media review, category tile overrides, landing-page hero images editable in admin.

**Hardening (review findings → done on `main`):**
- **P0 auth** `019498b1` — `requireCityAdmin` on `/api/admin/media`, offers/update, menu-preview, secret-menu, placeholder-custom/variant; media writes stamp `uploaded_by` / `curated_by` / `archived_by`. Unauth → 401 verified.
- **P1 framing** `a0d5f067` — public cards match editor: manual/centre use CSS focal+zoom on optimised `c_limit` derivatives (no broken `g_xy_center` 0–1); contain uses `c_limit`; auto cover keeps `g_auto`. Founder confirmed looks correct.

### 🔴 MUST DO (media leftovers — do before calling media “finished”)
| ID | Item | Why |
|---|---|---|
| `media-fk-delete-conflict` | Fix `ON DELETE SET NULL` vs ownership CHECK on `media_assets` | Deleting an offer/business with curated artwork can fail |
| `media-provider-metadata` | Detect provider on `createMediaAsset` (not always `cloudinary`); enforce allowed upload/URL policy | Metadata + upload trust |
| `r2-qr-install-attribution` | Partner/taxi QR install tracking + reviewable cents rewards | Jerry’s request; Acquisition R2 |

Prefer: archive-or-clear media in app before parent delete, or explicit CASCADE — don’t blind-ship without deciding retention.

---

## 📧 Email Suite — MERGED TO MAIN (Aug 8, 2026)

**Plan:** `/Users/qwikker/.cursor/plans/email_suite_architecture_13e5e711.plan.md`  
**Shipped:** `main` @ `de0a4543` (was `feat/email-suite`). Roadmap: `email-suite-phase1`…`phase2b` **done**; `email-suite-phase3` (Inbox/Receiving) **pending ops**.

### Live / usable now (outbound)
1. **Migration** `supabase/migrations/20260808120000_email_suite_foundation.sql` — applied on the DB we’ve been using; **confirm prod Supabase if separate**.
2. **Logger** — `logMeta` on `sendFranchiseEmail`; claim invites + Suite sends land in History.
3. **Suite UI** — Control Center **Email Suite** (New badge): Inbox · History · Templates · Campaigns · Automations · Settings.
4. **Templates (ops-facing only)** — claim invitation (1:1), claim reminder (campaigns), completion, free→trial, Spotlight, try loyalty, offer suggestions, trial extension, custom. System receipts (welcome / approval / loyalty ready / digest) hidden from picker; loyalty ready auto-sends on admin activate.
5. **Recipient picker** — search business / name / email; CRM envelope + Send email deep-link to Suite.
6. **Campaigns** — claim reminder for unclaimed; hard cap 50; dry-run → preview ack → confirm.
7. **History** — full HTML snapshot; search subject/email/business; “Show all” clears business filter.
8. **Webhooks / unsub / digest cron** — `/api/webhooks/resend`, `/e/unsub/[token]`, Mon 08:00 UTC digest (off by default). Bournemouth: send verified + webhook secret saved; **Receiving MX still Failed**.

### Still needed
| Item | Notes |
|---|---|
| **Phase 3 Inbox** | Cloudflare MX for `bournemouth` → Resend inbound; then test `hello@` → Inbox |
| **Other cities** | Resend domain + webhook `whsec_` + Receiving when ready |
| **Automations** | Wire `trial_ending` / `completion_nudge` crons |
| **Deliverability** | List-Unsubscribe header, bounce auto-pause, HTML purge, `db-retention-cost-guard` before volume |
| **Training** | Update EP3 (still says leave Receiving off) |
| **Users audience** | Not started |

### Same commit extras (not Email Suite)
- Acquisition **Claimed** KPI fixed (invite cohort, not unclaimed-only list).
- AI Companion: conversational stopwords + plural stem for menu match; rentals filtered on food queries (Mel’s / burgers).

---

## 🚨 Zanzibar AI chat inventory — FIXED (Aug 10, 2026)

Jerry / CHE Rock / Splash / Basil: chat said “not in listings” while Discover/admin showed LIVE.

**Root cause:** T1 `chat_eligible` needs `status=approved` + paid/trial sub. Admin **Starter** updated subscription/`plan` but left `status=claimed_free` (Starter was missing from the paid-tier guard). Free claims never required menu items, but T2 `lite_eligible` required `menu_preview ≥ 1` → Zanzibar T2=0. Unclaimed T3 still worked (Aug 4 CHE fix).

**Shipped:**
1. `updateBusinessTier`: trial/starter/featured/spotlight → `status=approved` + `visibility=ai_enabled`
2. `downgradeToFreeListing`: → `claimed_free` + `plan=free` + `visibility=discover_only`
3. T2 view broadened: menu OR live offer OR description ≥40 chars (drop hard `business_tier=free_tier`)
4. Chat name lookup: exact-token scoring + service-role detail fetch + profile fallback
5. Data: stuck paid+`claimed_free` → approved; free rows with fake `plan=starter` and no sub → `plan=free`; CHE `visibility=ai_enabled`

## 🐛 AI chat dish retrieval + bare-name lookup — IMPROVED (Aug 8–10, 2026)

**Dish/menu (Aug 8):** `scoreMenuPreviewMatch` stopwords + plural stem; food sanity filter excludes scooter/rental/hire.  
**Bare-name / Kibanda (Aug 9–10):** conservative bare-name candidates, generic category words filtered, DB fallback across eligibility tiers, city-gated detail retrieval (Jerry: Kibanda Beach Club “could not find details”).  
**Still open:** cold fuzzy misspellings for some venues; regression-test Kibanda / beach club / menu phrasing. Roadmap: `ai-chat-menu-item-retrieval`.
...
- [ ] **dennis-22 — Featured-item currency (£ hardcode fix).** On `kefalonia.qwikker.com`, open a business with featured items: prices should read e.g. `From €45/day` / `Free` (NO leading `£`); a bare-number price shows `€`. On a GBP city (Bournemouth) bare numbers still show `£`. *Dennis pointed this out — include in reply.*
- [ ] **dennis-04 — Submit confirmation.** `/dashboard/profile/preview` → "I'm Happy - Submit!" shows a confirmation modal (live vs under-review wording) before going to action-items.
- [ ] **dennis-06 — Map icon.** Admin Live Listings → green map pin opens the actual business listing (or name+address search), not a bare dropped pin. Test a business WITHOUT a google_place_id.
- [x] **dennis-17 — Business photo cropping (DONE-ish, possible further refinement).** Photos now fill their frame cleanly with a centered crop (`object-fit: cover; object-position: center`) across cards, admin preview modal, and the detail hero — no more top-cut/empty space. **Root cause (why it looked unfixable):** (1) a global *unlayered* `img { height: auto }` rule in `globals.css` was overriding Tailwind's `h-full` on every image (unlayered CSS beats layered utilities in Tailwind v4), so `object-fit` had no effect and `overflow-hidden` chopped images; (2) Turbopack dev wasn't reliably regenerating newly-added utility classes, so class-based fixes silently never reached the browser. **Fix:** moved the `img` reset into `@layer base` AND set sizing/fit via **inline `style`** on the carousel `<img>`s in `components/ui/image-carousel.tsx` (inline styles can't be overridden by any stylesheet). The `fit="contain"` (full-image + blurred backdrop) mode still exists in the carousel if we want it per-context later. **Possible refinement:** center-crop can still clip content sitting hard against an image's top/bottom edge (only seen on AI demo seed images with baked-in text); revisit per-image (crop anchor, taller frames, or contain) only if a real uploaded photo crops badly. *Mention the "done, minor refinement possible" caveat to Dennis.*
- [ ] **dennis-10 — QR name + description (create + edit).** Admin → QR Management → QR Code Generator now has optional **Name** and **Description** inputs on create (blank → auto-generated code name / `"<subtype> QR code"`). Existing codes can also be edited: the per-code **Edit** button (was "Edit URL") opens a modal to change Name, Description AND destination URL together. Persists to `qr_codes.name/description`. New server action `updateQRCodeDetails` in `lib/actions/qr-management-actions.ts`; UI in `components/admin/comprehensive-qr-dashboard.tsx`.
- [ ] **dennis-05 — Change placeholder image (PARTIAL — REOPENED Jul 7).** The admin change persists (`/api/admin/businesses/placeholder-variant` writes `placeholder_variant`) and shows in **Admin user-view** + **Discover** (both read the column via `getPlaceholderVariationWithOverride`). BUT it is IGNORED on: (1) **home feed** — `lib/home-feed/feed-builder.ts` doesn't SELECT the column and `getBusinessImage()` → `getPlaceholderUrl()` is hash-only; (2) **business detail** — `app/user/business/[slug]/page.tsx` doesn't SELECT the column and falls back to `/placeholder-business.jpg`. So Dennis sees the change in admin but not on the live page. **FIX:** SELECT `placeholder_variant` in feed-builder + business/[slug], and use the override-aware placeholder (`getPlaceholder`/`getPlaceholderVariationWithOverride`) instead of the hash-only `getPlaceholderUrl` in `getBusinessImage`. No DB change.
- [ ] **dennis-07 — AI-eligible un-toggle + tooltip.** Admin → Unclaimed tab. Default view: select not-yet-eligible listings → **Make AI eligible** (grants Tier 3 fallback). Click **Show AI Eligible** filter → now select already-eligible listings → button becomes **Remove AI eligibility** (revokes). Confirmation modal shows enable vs disable copy; success banner reads "now/no longer AI eligible". Tooltips added on the action buttons + the CRM card "AI Eligible" tile explaining it's text-only chat fallback and doesn't affect Discover. `POST /api/admin/bulk-ai-eligible` now accepts `approved: boolean` (defaults true). Files: `app/api/admin/bulk-ai-eligible/route.ts`, `components/admin/admin-dashboard.tsx`, `components/admin/ai-eligible-confirmation-modal.tsx`, `components/admin/comprehensive-business-crm-card.tsx`. No DB migration (column/view already support false).
- [ ] **Expired-trials public leak fix.** The 8 expired trials must NOT appear in Discover / home feed / business detail / landing Live Offers, while active businesses still show. (roadmap `prod-expired-trials-public-leak`)
- [ ] **Admin CRM card responsiveness** (not a Dennis item). Names show in full (wrap, no "The Ba…" truncation) and stat tiles/height look right at mobile + medium (2-up) + full-screen widths.

## Current Status (Updated June 24, 2026)

- **🧭 DENNIS AUDIT + DEPLOY STATE (Jul 20) — read this before "fixing Dennis".** The Jul‑7 TEST LIST at the very top of this file is **STALE** — the roadmap plan is the source of truth and most items are already DONE in code. **CRITICAL DEPLOY REALITY:** `origin/main` is at `dadf665c`; the whole **"Dennis fixes batch" is a single commit `c43a9321` on branch `feat/ai-offer-engine` that has NEVER been merged to main → NOT live on any city.** On top of it sits all the uncommitted Acquisition Engine + email work, touching some of the SAME files (`feed-builder.ts`, `price-formatter.ts`, `user-business-detail-page.tsx`, `recent-activity-actions.ts`) — so Dennis fixes and acquisition work are **entangled in one branch**; a clean "deploy just Dennis" will need untangling.
  - **DONE in code (committed in c43a9321) — need LIVE TEST + deploy:** dennis‑01, 03, 04*, 05*, 06, 07*, 10*, 11 (no‑code), 16* (needs real‑inbox test), 17*, 18, 20 (code done, needs real e2e test), 22*, 23. (* = roadmap flags ⚠️ needs live test.) Plus `prod-expired-trials-public-leak` (public surfaces committed; the **dashboard** ticker + feed offers/events fix is uncommitted, this-session).
  - **NOT built — small:** dennis‑13 (non‑Latin/Greek slug → transliterate before slugify; cosmetic/SEO), dennis‑19 (AI carousel card — decide re‑enable Tier‑1 vs delete dead path), dennis‑21 (per‑city GTM/GA4 injection into `<head>` from franchise config + consent; Kefalonia `GTM‑5L8QGL5N` — small/med).
  - **NOT built — bigger:** **Events system = dennis‑02 + 09 + 14** (spec'd in `CITY_EVENTS_ADMIN_SPEC.md`, the "serious thought" piece), dennis‑15 offer **redemption**, dennis‑08 offer **expiry automation**, dennis‑12 UI **i18n** (content + AI already multilingual; only the English chrome needs localising — large, NOT urgent/blocking).
...
- **⭐ SPEC'D (Jul 22) — Prospecting "Present Mode" / door-to-door demo page (the physical twin of the claim email).** Founder idea for franchisees going door-to-door: instead of a cold email, walk in and show the owner a **personalised, full demo of their own listing** on a phone/tablet — "what you see is what you get, claim it now". Directly answers **Philippe's "prospecting angle"** question. **~80% already exists:** the Acquisition Engine already generates the full grounded draft (`business_enrichments`: description, tagline, REAL featured items, 3 offers + rationale, Google rating + review_count) and we have **publish-on-claim** + the business detail page + claim flow — Present Mode is mostly a **business-facing reskin of that draft** + an "unlock" showcase.
  ...
  - **Honesty rule intact:** real listing content stays grounded (no invented menu items — empty if no menu found); the loyalty/analytics/push panels are explicitly *"example of what you'd get"*, not claimed as real data.
  - **Build notes:** do it on its **OWN branch** (don't tangle with the un-deployed Dennis batch, like we did for the wedding page); **mobile/tablet-first "present" UX** (full-screen, swipeable sections); pre-loadable before walking in (patchy door-to-door signal) → offline-friendly polish later. Likely files: `app/demo/[token]/page.tsx` + an admin "Present" entry on `acquisition-pipeline.tsx`, a token sign/verify util, reuse `generate-acquisition-draft` / `business_enrichments`, new mock components (loyalty card / analytics / lock-screen push). **Effort: medium** (mostly reskin + token route + mock panels). **Status: SPEC'D — build AFTER the Dennis batch is deployed.**
  - **COPY / CONTENT DIRECTION (decided Jul 22) — every section = benefit HEADLINE + one plain-English line + a clearly-labelled `example` stat. Verb-led ("See… Spot… Reach… Turn…"), no jargon, short enough for the franchisee to talk over. Personalise real tokens (business name; Google rating + review_count are REAL not "example"). Mark all mock numbers `example` (keeps the honesty rule — they become real once live).**
    - **Your listing (real, top of page):** *"You're already on Qwikker — this is your live listing."* → real grounded content + the personal hook (*"287 reviews mention your Sunday roast"*). (It's genuinely already discoverable — see REALITY CHECK below.)
    - **Featured dishes:** *"Your signature dishes, front and centre."* → The dishes people rave about, pulled straight from your menu & reviews (with photos) so diners crave before they arrive. `example: most-mentioned — Babi Guling, Nasi Campur, Fresh Coconut`.
    - **Secret Menu:** *"A secret menu only Qwikker unlocks."* → Hidden, off-menu items diners discover and unlock in-app — the insider find that gets shared and pulls people in. `example: unlock 'Off-Menu Sambal Prawns'`. (Ties to Secret Menu Hunts FT-13/11.)
    - **Offers:** *"Offers that actually bring people in."* → 3 ready-made offers built from your reviews & menu, each with a one-line 'why this works'.
    - **Analytics (hero: "See what works, when it works, and who it works for" — sub: "Every claim, save and visit becomes plain-English insight — no spreadsheets."):** WHAT works — *"Your top performers"* (which offers/dishes get claimed & redeemed most — do more of what sells; `example: Sunday Roast — 142 claims`). WHEN — *"Your busiest (and quietest) moments"* (spot dead nights, drop an offer to fill them; `example: quietest = Tue/Wed 3–6pm`). WHO — *"Who's coming back — and who isn't"* (new vs regulars + people who saved you but never visited; `example: 18 saved, haven't been in`).
    - **Loyalty:** *"Turn first-timers into regulars."* → A digital stamp card that lives in their Apple & Google Wallet — no app, no paper card to lose. They come back to earn; you see every visit. `example: Buy 9, get the 10th free — 34 cards started`.
    - **AI concierge / knowledge injection:** *"Be the answer when someone asks 'where should we go?'"* → Your menu, offers & vibe feed Qwikker's AI concierge, so when a visitor asks 'best roast near me' or 'somewhere for date night' it recommends YOU with your live offer attached — and it nudges the right people your way (*"you saved The Ivy but haven't been — 20% off tonight"*). `example: recommended to 63 people who asked this month`.
    - **Food-smart AI (feature this in the Food-first preset, e.g. Bali):** *"Ask it anything about food."* → 'gluten-free pizza near me', 'best vegan brunch', 'something spicy under £10' — it understands the craving AND points to the right DISH at the right place. It learns each diner's tastes, allergies & favourites, so answers sharpen every visit (and it can steer them to your signature/secret dishes). `example: 214 food questions answered — 63 sent your way`.
    - **Push notifications:** *"Reach them at the perfect moment — on the lock screen."* → Send a branded offer straight to their phone; fire it when you're quiet or when they're walking past. No 'open the app' — it lands like a text. `example: '£5 off mains til 8pm' → 210 nearby reached, 41 opened`.
    - **Branded wallet pass (Living Pass):** *"Your brand, in their pocket — and it comes alive."* → A wallet pass in your colours & logo whose face BECOMES your current offer, updates itself, and can surface when they're nearby. `example: pass mock — your logo + "Claim £20 off — 5 min walk away"`.
    - **Closer / tiers:** *"You're already on Qwikker and being found — claim it free to take control & switch on your offers, then add loyalty, push & analytics any time."* (Free vs Spotlight/Trial framing.)
  - **PRESENTATION MODE DROPDOWN (Jul 22 — decided SIMPLER than per-city auto).** No automatic city/vertical detection and NO changes to public surfaces (deliberately out of scope). Instead, a small **dropdown in Present Mode** lets the presenter pick the emphasis for the business in front of them — e.g. **Food-first** (leads with Featured Dishes + Secret Menu + food-smart AI; ideal for Bali's 'food hub' push), **Services/Treatments** (spa/salon/clinic), or **General** (default). It just reorders/toggles section visibility — presenter switches as needed. Cross-refs (content only): Secret Menu Hunts (FT-13/11), Atlas 'discovery by craving'.
  - **REALITY CHECK (Jul 22) — the base listing is ALREADY LIVE.** To showcase a business it's been imported, and imported **unclaimed listings show in Discover by default** — so claiming does NOT "make it live". What claiming actually flips: (a) **ownership/control** (`owner_user_id` set → they can edit), (b) **publishes the ENRICHED upgrades + activates the offers** (rich description/dishes/offers are held back until accept/claim — publish-on-claim), (c) **AI eligibility / full participation**, (d) opens the **trial** for loyalty/push/analytics. This makes the pitch STRONGER: *"you're already being found here — claim it to take control before it just sits unclaimed."*
  - **⚠️ OPEN DECISION — what "Claim now" grants (shapes the close):** **Option A (recommended):** claiming = **take ownership + publish the enriched upgrades + activate the offers** (base listing already live from import); loyalty/push/analytics become the **"start your free trial and I'll switch them on now"** upsell — cleanest, protects the tiers, flows into trial. **Option B:** same, but claiming **also auto-starts the trial** so everything they saw turns on at once — maximum wow, but gives paid features away at hello. Decide before build; either way demo → claim → (trial) should feel like one continuous moment.

- **🟢 DONE LOCALLY (Jul 18) — Acquisition Engine: email reliability + big UX simplification (UNCOMMITTED/undeployed).** Two threads this session:
  - **Email discovery ("look harder", still only ~1/10–1/15 landing):** new shared `extractEmailsFromHtml()` in `lib/utils/email-filter.ts` used by every scrape path — decodes `mailto`, **Cloudflare `data-cfemail`/`email-protection#hex`**, JSON-LD `"email"`, HTML entities (`info&#64;domain`) and human obfuscation (`info [at] domain [dot] com`). `lib/listing-engine/fetch-website.ts` now (1) crawls **/contact + /about** pages for emails (not just homepage), (2) **no longer bails when the homepage is blocked/JS-only** — falls through to guessed contact/menu paths and still returns if only an email was recovered, (3) hunts **services/treatments/price-list** pages for non-food businesses (spa/salon/gym/clinic) via category-aware `MENU_HINT`/guessed paths so treatments flow into featured items. `app/api/admin/find-business-email` checks 5 contact paths. **Website recovery:** new `lib/listing-engine/fetch-place-details.ts` pulls `websiteUri` from Google Place Details in the SAME (cost-neutral) call as reviews; `generate-acquisition-draft.ts` uses a recovered website when none is on file and **persists it to the profile** (the #1 "no email" cause = no `website_url` on file). Honesty rule intact (never fabricate items/offers; no readable menu → empty featured items). **Remaining lever = a paid finder (Hunter/Apollo) for named owner emails** — scraping can't beat contact-form-only / Facebook-only / no-address sites. (TripAdvisor confirmed a dead end: no published email + heavy anti-bot.)
  - **UX redesign (founder: "way too busy"):** the per-step card grid is replaced by **3 result buckets** under the 1‑2‑3 steps — **Send-ready / No email / Needs attention** (criteria step-aware: pre-enrich = raw signals, confirm = confidence+email, invite = email). Buckets are single clickable rows → open a **full-width review** (old right-drawer → full-tab panel; taller email preview). Removed Start-a-campaign, search box, filter toggles, and per-card action buttons. **Gated bulk send** = new `/api/admin/offer-engine/send-bulk` + shared `lib/listing-engine/send-claim-invite.ts` (also refactored the single-send route onto it): publish-then-send, skips claimed/sent/no-email, capped 200, behind a confirmation modal listing every recipient. ⚠️ This intentionally **reverses the old "no bulk send" rule** — the modal + skip-already-sent is the safety gate.
  - **⏳ Uncommitted** — typecheck clean on all changed files; verify in-app then commit + deploy.

- **🔴 URGENT — NEEDS INVESTIGATION — Partner Feedback Batch (Dennis, Kefalonia — June 30):** 11 items logged in the roadmap plan file as `dennis-01` … `dennis-11`, all status `pending`, all flagged **URGENT / NEEDS INVESTIGATION**. Summary: (1) **claim accepted an expired verification code** — enforce code TTL at submit; (2) **no events submission page / no CSV-PDF bulk upload** for orgs/municipalities; (3) **vibe tags are restaurant-skewed** — make tag options adapt to business type; (4) **"I'm Happy – Submit" gives no confirmation** (it does go live, but silently navigates to profile); (5) **can't change placeholder images** from Admin "User View" → "change image"; (6) **Live Listings map icon opens a bare map pin**, not the business/GBP; (7) **"make AI invisible again"** — ambiguous, needs clarification then an admin toggle; (8) **claimed-offer 7-day expiry** — *no automated expiry exists today* (status `expired` never set, wallet copy says 12h but nothing enforces it) — decide behaviour + build it, not a manual WalletPush rule; (9) **18 event PDFs ignored by AI** — *by design* (event queries bypass the KB and read only `business_events`); fix = ingest as structured events via CSV import and/or scrape `inkefalonia.gr/agenda`; (10) **QR codes need name + description in the UI** — columns already exist (`qr_codes.name/description`), gap is UI only; (11) **business entry-route confusion** (`/claim` vs `/business-signup` vs `/onboarding`) — `/claim` is the correct primary link; standardise messaging. Plus `dennis-12` **multilingual/Greek-German content** — *already works* (no English-only validation; AI is multilingual via GPT-4o, confirmed by Dennis); remaining gaps are English-only UI chrome (no i18n) + English-biased ranking helpers — proper UI localisation is the real future work; and `dennis-13` **non-Latin URL-slug fix** (pure-Greek names fall back to UUID URLs).
- **🔴 URGENT — NEEDS INVESTIGATION — Dennis Follow-up Batch #2 (July 7):** roadmap items `dennis-14` … `dennis-21` (plus clarifications folded into `dennis-02/07/08`). Highlights: `dennis-07` **CLARIFIED** — it's the "Make AI eligible" toggle on unclaimed listings (= approve an imported listing to appear in AI chat as a Tier-3 *text-only* fallback via `admin_chat_fallback_approved`); Dennis wants to understand it + be able to turn it OFF again — *there is currently no un-eligible action*. `dennis-14` **link events to a venue/business** (pick from listings, claim-style). `dennis-15` **offer redemption** — *honest finding: there is NO in-store redemption process at all* (status `redeemed` never written, "Add to wallet" is mislabeled "redeemed"), plus claim-amount single/multiple isn't enforced (behaves single-use) and a 2nd claim overwrites the pass's single offer slot. `dennis-16` **free→trial nudge email** (60-day free trial). `dennis-17` **business photos top-cropped/cut off** — center object-position. `dennis-18` **onboarding on an already-listed unclaimed business errors** (route to /claim instead). `dennis-19` **AI carousel card is built but hardcoded OFF**. `dennis-20` **forgot-password BROKEN** (regression on 0.1 — missing PKCE `/auth/callback` code exchange; reset link errors; also verify city-subdomain redirect allowlist). `dennis-21` **per-city GTM/analytics injection** (Kefalonia `GTM-5L8QGL5N` — needs app-level per-city container support, WalletPush SEO settings don't apply). Full per-item detail in the roadmap plan file.
- **🟠 NEW (Jul 7) — `dennis-23` Town dropdown is hardcoded to Bournemouth (multi-tenant bug).** The business Profile "Town" select is driven by a single hardcoded constant `BUSINESS_TOWN_OPTIONS` in `types/profiles.ts` (Bournemouth/Poole/Christchurch/Wimborne/Ferndown/Ringwood/New Milton/Other), reused for EVERY franchise — so a Kefalonia business is offered Dorset towns (Dennis screenshot). Used in `components/dashboard/clean-profile-page.tsx`, `components/dashboard/business-info-page.tsx`, `components/admin/business-creation-modal.tsx` (also onboarding/founding-member/claim forms carry their own copies). There is NO per-city town source today: the franchise geography system was collapsed and `getFranchiseAreas()` is deprecated (returns `[city]`). **FIX OPTIONS (needs a quick decision):** (a) add a per-city `towns`/`areas` list to the franchise/city config (e.g. `city_franchises`/`franchise_crm_configs`) and populate the dropdown from the current city — most correct, some DB work; (b) replace the fixed dropdown with a free-text input or derive the town from the existing Google address autocomplete (address field already uses Places) — fastest, least structured; (c) hybrid: Google-autocomplete-driven town with a per-city curated shortlist. Affects onboarding, claim, profile edit, admin create, and any per-town filtering/ranking that assumes Dorset values.
- Full per-item detail (file paths + findings) in the roadmap plan file.
- **⭐ FLAGSHIP BUSINESS-VALUE (Jul 10) — Business Autopilot.** Master spec: `BUSINESS_AUTOPILOT_SPEC.md`; roadmap `business-autopilot` + `ai-prospect-offers`. Qwikker acts as an autonomous marketing dept per business and PROVES ROI. Unifies BV-1/2/5/7/10 + FT-1/6 + Goal-Based Autopilot + the AI-offer idea into one loop: AI drafts → business **one-tap approves** (Assistant model, decided) → auto-publish to listing + wallet → timed/geofenced push → track claims→redemptions→visits → "ROI this week" digest → suggest next → repeat. Thesis: businesses want **outcomes + proof**, not tools; the AI-offer-on-claim is **Act 1** of a lifelong relationship. Data foundation already real. **Spine order:** ROI digest (retention) → AI offer generation (`prospect_offers` table) → automated push → loop orchestration. Biggest whitespace = **Named Customer Intelligence** ("8 saved never visited → nudge"). New monetization: sponsored boost, prepaid vouchers/GMV, "quiet now" real-time offer. **NOT BUILT (spec-first).**
- **⭐ PRE-LAUNCH WANT (Jul 9) — Arrival Layer: ride to the offer.** Roadmap `arrival-layer-ride-booking`; detail in `QWIKKER_VISION_AMBIENT_AGENTIC_CONCIERGE.md` §7a (API access verified Jul 9). Concierge proactively offers a ride to the deal ("7 min from 50% cocktails at Pablo's — book a ride?"). **Pre-launch build = intelligence + TWO-TIER handoff, ZERO partnership:** (1) ETA/"7 min away" via Google Distance Matrix (own key, global) + governance; (2) **Tier 1 native deep-link** (destination pre-filled, real price in-app, one tap) for **Uber/Bolt/FreeNow/Gett** (Europe); (3) **Tier 2 Google Maps "Rides" fallback** (`dirflg=x`, aggregates Grab/Gojek/DiDi/etc.) for **all of Asia** where no open deep link exists. Per-city config picks tier+provider. **Corrected:** Uber price-estimate endpoint now needs Uber approval (can't print exact £ in own chat pre-approval; deep-link shows it inside Uber anyway); **Grab/Gojek/DiDi have NO open deep link** (Grab = full partnership) → hence the Maps fallback for Asia. V2 = full in-chat booking via Uber Rides API (Uber Full Access + per-user 'Connect Uber' OAuth + payment on file — parallel track, don't block launch). Future = sponsored rides via Uber Vouchers. **NOT BUILT.**
- **⭐ NORTH STAR (Jul 9) — Qwikker Vision: the Ambient, Agentic Local Concierge.** Full doc: `QWIKKER_VISION_AMBIENT_AGENTIC_CONCIERGE.md`. Thesis: "Qwikker knows what your town wants tonight — and acts on it, automatically, for both sides." The **pass is the primary surface** (living/morphing states + silent-morph vs selective-push, both already supported by the WalletPush field API), the **AI is agentic** (an MCP/action layer = the hands), and the **moat is a per-city real-time taste-and-redemption graph** (every save/claim/redemption/vibe/Atlas visit feeds it — redemption truth is the critical faucet, hence sequenced first). Two flagships: **demand** = Night-Planner living pass (sequenced multi-stop night in the wallet, FT-4); **supply** = Goal-Based Business Autopilot ("fill my Tuesdays" → auto flash offer → taste-matched push → measured redemption; BV-5+FT-6, the killer Spotlight upsell). Bigger bets: serendipity push, tourist/arrival mode, cross-city taste portability, honest post-visit loop, selling the aggregate graph. **This unifies existing scattered items (BV-16, FT-1, BV-5, FT-6, FT-4, dennis-08/15, City Events, Acquisition Engine) under one north star + sequencing** rather than adding net-new scope. ⚠️ 3 WalletPush API questions for Dad gate how "alive" we can go (which visual fields are PUT-able; does the new generic/poster card expose action buttons via API; Apple relevance/semantic support). Governance (caps/quiet-hours/consent) is load-bearing, build first.
- **⭐⭐ Living Pass (iOS 27) + Offer Redemption — Layer 1 SHIPPED (Aug 9–10); Layer 2 still pending.** **Layer 1 (device-agnostic)** done via Save/Redeem R1 — see section above: activations, 30/60/120 window, countdown, mid-window + expiry vibe pushes, single-use RPC, replace confirm. Answers Dennis `dennis-08`/`dennis-15`. **Layer 2** (Poster Generic morph / geofence / pass control room) still NOT BUILT — needs `LIVING_PASS_SPEC.md` + `WALLETPUSH_REQUIREMENTS.md` + Dad/platform APIs. Older notes on 12h theatre / Sample AI_Url re-seed / SeahorseOne sender remain relevant for Living Pass work.
- **🟢 SPEC'D (Jul 9) — City Events (Admin) — covers `dennis-02`/`dennis-09`/`dennis-14`.** Full spec: `CITY_EVENTS_ADMIN_SPEC.md`. Admin "City Events" feature: single-add + **CSV bulk import** that creates individual live event listings, with **assign-to-business later** (claimed/paid/unclaimed). **Design fits the system with NO schema migration:** writes to the STRUCTURED `business_events` table (NOT the KB — the existing KB-tab "Add Event" writes to `knowledge_base`, which the AI's event-query path BYPASSES = the root cause of `dennis-09`, i.e. those events/PDFs never answer "what's on"). Because `business_events.business_id` is NOT NULL and city derives from the linked business, unassigned imports anchor to a **per-city "What's On in {City}" host business** (the holding pen; must be excluded from Discover/feed/AI-business surfaces — only its events show); "assign to business" = repoint `business_id` + re-sync KB (this IS `dennis-14`). Admin imports go in as `status='approved'`. `dennis-09` resolved for free once events are structured (no AI change). Phases P1 core-write → P2 CSV → P3 assign → P4 cleanup(relabel KB "Add Event") → P5 optional `inkefalonia.gr/agenda` scrape. **NOT BUILT — spec only.**
- **🟠 NEW (Jul 9) — Social Wizard: honest current state + grounded rebuild plan (precedes BV-6 V2).** *Reconciles with `BV-6 Social Wizard V2` — my plan is the V1.5 rebuild that MUST land before the Meta-publishing V2 vision; publishing stays deferred to V2.* **REAL STATE (verified in code, not the rosy status docs):** (1) it's fully walled off — both `app/dashboard/social-wizard/page.tsx` and `app/business/social-wizard/page.tsx` render only a **"Coming Soon — being rebuilt"** screen; (2) the generator is **text-only** — `/api/social/ai/generate` returns just `{caption, hashtags[], template:"general"}` (the "template" is a bare string label, no real design output); (3) **two parallel half-built implementations** exist and conflict: newer Fabric.js `app/business/social-wizard/*` + `/api/social/ai/*` (Zustand `lib/social-wizard/store.ts`) vs older `components/dashboard/social-post-builder.tsx` + `/api/social-wizard/*`; (4) **DALL·E is wired but effectively dead** — `/api/social/ai/background` (dall-e-3, abstract mood backgrounds only, buried button in `TextStylePanel`, needs a per-franchise OpenAI key that's likely unset) and DALL·E 3 can't render legible text anyway. **ROOT CAUSE of "terrible generic output":** it's architectural — AI writes a caption + generic hashtags rendered as a text box on a blank/AI canvas; it ignores the business's OWN photos; prompt has thin context/no brand voice. **RECOMMENDED VISION = hybrid:** deterministic **branded template engine using the business's own photos + AI copy** as the CORE (looks designed, cheap, no external dep), with modern AI image models (**Ideogram** best-in-class for legible in-image text, or **GPT-image-1**/**Recraft**) as a **"no photo" fallback only** — and even then, text is always overlaid by the template renderer so it stays crisp. **REBUILD PLAN (phased):** **P0** consolidate — keep `app/business/social-wizard/*` + `/api/social/ai/*`, delete the older `social-post-builder.tsx` + `/api/social-wizard/*`; keep behind a feature flag with the Coming-Soon wall up. **P1** copy engine v2 — change AI output to **structured fields** (`headline/subhead/cta/caption/hashtags`), feed real offer/event/menu data + derived brand-voice + enforced local hooks + few-shot examples + "improve" refine actions (*this alone kills the generic feel*). **P2** template render engine — server-side PNG via Satori/`@vercel/og` `ImageResponse`, 3–4 templates (offer/event/menu/general) consuming structured copy + business photo + franchise colours + logo, 1080×1080 & 1080×1350 (*this kills the placeholder look*). **P3** photo selection from existing `business_images` + category stock fallback. **P4** AI image fallback (Ideogram/GPT-image) for no-photo — background only, text overlaid by P2. **P5** Spotlight campaign packs (5-post series via `/api/social/ai/generate-campaign` through the template engine). **DEFERRED to true V2 (BV-6):** scheduling-as-reminders first, then real Meta Graph API publishing (needs Meta App Review + Business Verification — long lead; start that clock separately). **DECISIONS NEEDED before P2:** render tech (Satori recommended vs headless browser); template/brand direction (fonts, Qwikker green, logo lockups — or build sensible defaults); first AI image provider for P4 (Ideogram vs GPT-image-1, no rush). Tier gating reuses `lib/social-wizard/featureFlags.ts` (free/starter locked; trial/featured limited; spotlight full). *Recommendation: start P0+P1 first (highest pain, lowest risk, wall stays up).*
- **✅ FIXED (July 7) — Expired trials leaking into public views (post-restore):** After the Supabase restore, 8 EXPIRED-TRIAL businesses reappeared in user **Discover "Featured"**, **home feed**, **business detail**, and the landing **"Live Offers"** — while the **admin correctly showed them as Expired** and the **AI correctly hid them**. ROOT CAUSE (confirmed, *not* data corruption — subscription `updated_at` are all old Jan/Apr, untouched by the restore): `business_subscriptions` has `UNIQUE(business_id)`, so PostgREST embeds it as a **single object, not an array** — the public filters guarded with `Array.isArray(...)` / `.length` fell through to their `return true` (show) branch and never ran the expiry check. Admin was immune (it already normalises the shape); AI/Offers immune (use the `business_profiles_chat_eligible` SQL view). Same bug class already fixed for the **loyalty** join in commit `50eebd6a` (Feb 26) — the subscription filters were just never given the same treatment. FIX (code only, **no data changes, no cron re-added**): new pure helper `lib/utils/trial-status.ts` (`normalizeSubscription` + `isBusinessTrialActive/Expired`) applied in `app/user/discover/page.tsx`, `lib/home-feed/feed-builder.ts`, `app/user/business/[slug]/page.tsx`, and `app/page.tsx` (featured carousel **and** the Live Offers block, which had no trial filter at all — now joins `business_subscriptions` and filters it). **Deliberately minimal scope:** the helper hides ONLY genuine expired trials (`is_in_free_trial=true` + past `free_trial_end_date`), so the *only* businesses whose visibility changes are actual expired trials — nothing else is newly hidden (it does NOT treat `status='cancelled'` as expired, since a cancelled row can be a downgraded-to-free listing that should still show). The landing featured carousel keeps its own extra `cancelled`→hide rule to preserve prior behaviour there. Extend-trial / convert-to-paid un-hides a business automatically. **Risks/rollback + full write-up:** roadmap item `prod-expired-trials-public-leak`. ⚠️ Still needs a live verify on a city subdomain (the 8 should vanish from public views; active businesses stay).
- **⚠️ FIXED LOCALLY, NOT YET DEPLOYED (Jul 17) — Expired trials STILL leaking on USER DASHBOARD (July-7 fix was incomplete).** The Jul-7 fix migrated 4 surfaces to the shared `lib/utils/trial-status.ts` helper but **missed the logged-in dashboard**, so expired trials reappeared there. Same root cause as before: `business_subscriptions` UNIQUE(business_id) → PostgREST embeds a **single OBJECT**, and the old `Array.isArray(...)` guards silently no-op'd → fell through to "show". **FIXED (code only, no data/migration):** (1) `lib/actions/recent-activity-actions.ts` — the dashboard "New business joined / added offer / added secret menu" activity ticker (rendered by `components/user/user-dashboard-home.tsx`) had the broken `Array.isArray` guard in **3 places**; all now use `isBusinessTrialActive()`. (2) `lib/home-feed/feed-builder.ts` — the dashboard **Deals** (`fetchOffers`) and **Tonight** (`fetchTonightEvents`) rails weren't joining `business_subscriptions` at all; both now embed it + filter via the helper. **⏳ WON'T take effect on production until committed + pushed to `main` + Vercel deploy** (currently uncommitted local changes). **STILL MISSING the helper (secondary surfaces, NOT yet fixed — come back to these):** `app/user/events/page.tsx` (+ `upcoming_events_with_business` view), `/api/loyalty/discover` (Rewards page + dashboard loyalty rail), `/api/atlas/search` hydrate-by-id mode, `/api/user/notifications` business-name lookup, `/api/ai/add-to-wallet`, `app/user/saved` (shows stale saved businesses), and the dashboard/settings **event count** stats. Note: `unclaimed`/`claimed_free` legacy listings showing in Discover/feed is **intended**, not this bug. Roadmap: `prod-expired-trials-public-leak`.
- **⭐ FLAGSHIP IDEA (Jul 17) — "Walk past → your pass BECOMES the offer".** Consumer ad hook: *"Get live offers sent straight to your mobile wallet the moment you're within walking distance."* Business/monetization hook: *"Want your offer on someone's lock screen the moment they walk past your door? Only 10 spots in {city} — join the waitlist."* **KEY CONCLUSIONS (worked out Jul 17):** (1) **Two separate "10s":** per-USER 10 = Apple hard cap of `locations` geofences per .pkpass; per-CITY 10 "Proximity Spots" = a marketing SCARCITY choice (not a tech limit). Elegant alignment: sell ~10 scarce spots per city → load all 10 into every user's pass → walk past ANY paid venue → **app-less** lock-screen surfacing. Scarcity also protects the channel from spam + enables premium pricing. Reframe: **acquisition billboard, not loyalty** (they need not be existing customers). (2) **App-less vs needs-native-app:** lock-screen surfacing near a venue = app-less ✅ (silent, visual only, no sound); static per-location `relevantText` = app-less ✅ (use "moments away", can't compute live distance without app); remote **poster IMAGE update = app-less ✅** (empty-payload APNs push → Wallet re-pulls .pkpass — earlier "needs app for image" was WRONG). **The real blocker = the TRIGGER:** iOS handles geofence entry on-device and never tells our server, so auto-morphing the image at the exact venue on walk-past needs background location = **native app** (or on-site iBeacons). No custom/guaranteed sound without a native app. (3) **Shippable app-less V1 loop:** passive lock-screen nudge near a pre-loaded spot → user **taps** pass → opens with 2 iOS-27 action tiles ("Get directions" + "View/Claim offer") → the Claim tap **opens our URL = the server signal we were missing** → record claim + push image morph. Native app later adds dynamic geofence re-centering + true silent walk-past morph + sound. Full write-up + open WalletPush questions in roadmap `ft-1a-walk-past-living-offer` (expands `ft-1-geofenced-push`). NOT BUILT — feasibility spike pending.
- **🔴 MAIN PRIORITY — Unauthenticated Admin/Debug API Routes (VERIFIED LIVE June 24):** ~40 `/api/admin/*` routes (plus `/api/debug/*`) run service-role operations with **no auth check**. Verified live on bournemouth.qwikker.com (corroborated in Vercel logs): `GET /api/admin/contacts` returns business PII (names/emails/phones/admin_notes) with no login (200, "Retrieved 16 contacts"); `GET /api/admin/comprehensive-analytics` open; `POST /api/admin/contacts/update` is an **unauthenticated write** (empty body → its own 400, not 401). Controls confirm it's patchy not total: `/api/hq/franchises` and `DELETE /api/admin/delete-business` correctly return 401 (prior audits secured HQ/payments/claims/delete). Root cause: no central auth enforcement — `middleware.ts` only refreshes the session. Real GDPR/integrity risk (live UK/EU cities + US franchises pending); no evidence of exploitation. **Verdict: fix soon & carefully, not a panic.** DEBUNKED (not broken — do not touch): `app_users.city` CHECK (doesn't exist), `business_notifications` table (exists), atlas event mismatch (not emitted). Safest fix = **surgical per-route guards** copying the proven `delete-business` pattern (`lib/utils/admin-auth.ts`), NOT a blanket middleware (would lock out login/reset + Stripe callbacks). Rollout: read-only audit → branch/preview → per-city smoke test → incremental ship with instant rollback. Full detail + priority order in the roadmap plan file (`security-admin-route-auth`).
- **🚀 MAIN-LAUNCH PRIORITY — Qwikker Acquisition Engine (designed June 24-25):** Big multi-month build, NOT needed for the current soft/beta launch (going well with partners) but **essential for main launch**. Unifies outreach-campaign-tool + Promo Pack QR + QR consolidation into one two-sided funnel: **import → enrich (bulk email scrape+verify, CSV) → outreach (physical + email) → convert (claim + AI pre-built offer that publishes to the landing page on approval) → acquire users (in-store QR)**. Physical is the safe primary channel: per-business "you're on Qwikker" cards + in-store table tents/stickers with tracking QR, generated via VDP templates + `jsqr` preflight, routed by destination zone — **Lob (UK/US auto-mail), PostGrid/Stannp (EU auto-mail), Gelato (in-store products only), local print (hard regions)**. Billing = **centralised accounts + prepaid Stripe credit wallets per franchise** (turnkey/done-for-them, zero float risk, small markup = revenue). Email outreach must use a **separate domain from transactional Resend**. Open founder decisions: confirm providers per zone + accountant on reseller VAT/tax. Full spec in roadmap plan file (`qwikker-acquisition-engine`; email detail in `outreach-campaign-tool`).
...
- **BV-6 Social Wizard V2 (May 17):** PRD complete. Meta-verified social media management tool for Spotlight businesses. 6 phases: Meta OAuth, AI captions, hybrid image system, scheduled publishing, engagement analytics, Meta verification. Dev mode allows 40 Early Access businesses before verification. Full plan: `.cursor/plans/social_wizard_v2_prd_1773affa.plan.md`. **⚠️ SUPERSEDED-AS-SEQUENCED (Jul 9):** the built V1 is broken/generic and walled off — a grounded **V1.5 rebuild** (template engine + structured copy) must land FIRST; the Meta-publishing parts of this V2 stay deferred. See the "🟠 NEW (Jul 9) — Social Wizard" entry above for the verified state + phased plan (P0–P5).
...
- **DFY Listing Setup — "we help businesses who get stuck" (June 8):** Strategic feature line. Industry standard is **delegated access, never password sharing** (like Google Business Profile managers). One concierge "edit-on-behalf" engine, monetised three ways: **Phase 1 — Concierge Edit-on-Behalf** (free onboarding rescue; franchise admin completes/optimises a stuck or lazy business's listing — IN PROGRESS, design stage). **Phase 2 — Paid DFY Setup** (one-time fee / Spotlight perk + in-product "we'll finish it for you" CTA + magic-link login). **Phase 3 — Managed Listing** (recurring £X/mo: Qwikker adds offers, refreshes menus, posts social on the business's behalf — agency model). Phase 1 architectural decision RESOLVED (June 25): **Option A** — reuse the existing service-role, `userId`-parameterized business-edit actions (`lib/actions/business-actions.ts`, `seamless-updates.ts`) from a city-scoped admin route; reuse the `business_changes` pending-update flow for owner transparency/override. Option B (extend HQ impersonation to business owners) REJECTED — the edit actions already bypass owner auth, so impersonation is unnecessary and would require minting real Supabase owner sessions (security-sensitive) while making audit harder. CRITICAL: the new admin route MUST verify the target business is in the admin's own city (the edit actions accept any `userId` via service role). Originally framed as: build admin-side edit forms (Option A) vs reuse the existing HQ impersonation pattern (`app/api/hq/impersonate/route.ts`, `impersonation-banner.tsx`) extended to business owners so the rep edits via the business's own dashboard (Option B — far less code). All phases require admin auth + city scoping, an audit trail, and "managed by Qwikker" transparency. Full detail in the roadmap plan file.
- **Multi-Vertical Category Expansion (June 8):** DONE. Qwikker is no longer food-only — added 5 system categories: `rental`, `automotive`, `health`, `tours_activities`, `grocery`. Full audit first (nothing breaks): all 4 TypeScript-enforced exhaustive `Record<SystemCategory>` maps updated (`SYSTEM_CATEGORY_LABEL`, `CATEGORY_MAPPING`, tagline `BASE_TEMPLATES`, `PLACEHOLDER_LIBRARY`), Google-type + display-label classifiers extended, category-aware featured-items labels (no more "must-try dishes" on a motorbike rental), both onboarding business-type dropdowns, and the admin `update-category` route (replaced fragile substring map that had a latent `nightclub` bug). Removed `car_repair`/`car_wash` from `GLOBAL_DENIED_TYPES` so automotive can import. No DB migration (live `system_category` is unconstrained TEXT). Existing rows re-categorised via a `google_primary_type`-driven SQL (21→health, 3→tours, 2→rental, 1→grocery); sanity check confirmed 0 invalid/null across all 910 businesses. Build unaffected (`ignoreBuildErrors` on; 0 new type errors). **FOLLOW-UP (next week):** dedicated placeholder artwork for the 5 new categories — they currently reuse existing folders (rental→sports, automotive→other, health→wellness, tours→entertainment, grocery→retail). Files: `lib/constants/system-categories.ts`, `lib/constants/category-mapping.ts`, `lib/constants/category-placeholders.ts`, `lib/import/tagline-generator.ts`, `lib/utils/featured-items-labels.ts`, `app/api/admin/update-category/route.ts`, `components/simplified-onboarding-form.tsx`, `components/founding-member-form.tsx`, `lib/actions/signup-actions.ts`.
- **CRM Email Management + Claim Outreach (June 9):** DONE. Imported (unclaimed) listings rarely carry an email from Google, which blocked outreach. (1) The expanded CRM card now shows an inline **"Add email"** button on the empty Email row (and a pencil-edit when one exists). (2) Quick-action email buttons (collapsed icon + expanded "Email" button) no longer fire an empty `mailto:` — with no email they open a **"No email on file"** modal to capture one. (3) That modal has a **"Find email on their website"** helper that scrapes the stored `website_url` (homepage + `/contact`) for `mailto:`/email patterns, filtered against junk addresses (SSRF-hardened: blocks loopback/private/link-local/metadata hosts, 8s timeout). (4) A green **"Send Claim Email"** button on unclaimed cards sends a branded *"Claim your listing on QWIKKER"* invitation (shared `wrapInLayout` logo/shell) whose CTA deep-links to `/claim?business_id=<id>` — already validated + pre-selected by `/api/claim/preselect`, so owners skip the search step. Mirrors the completion-reminder pattern: **preview modal + mandatory approval**, sent via `sendFranchiseEmail` from the city subdomain Resend. All routes are admin-auth + city-scoped. Files: `app/api/admin/send-claim-email/route.ts`, `app/api/admin/update-business-email/route.ts` (zod-validated), `app/api/admin/find-business-email/route.ts`, `components/admin/add-email-modal.tsx`, `components/admin/send-claim-email-modal.tsx`, `lib/email/templates/business-notifications.ts` (`createClaimInvitationEmail`), `components/admin/comprehensive-business-crm-card.tsx`.
...
| 0.34 City KB Retrieval Fix | `lib/ai/embeddings.ts`, `supabase/migrations/20260603000000_add_search_city_knowledge_rpc.sql` (new DB function) | Low | **Bug:** city-level knowledge (`knowledge_base.business_id IS NULL`, e.g. admin-uploaded festival/event PDFs) was never returned by the AI — the live vector `search_knowledge_base` RPC INNER JOINs `business_profiles_chat_eligible`, structurally excluding null-business rows (e.g. Alicante bullfighting PDF). **Fix:** new DEDICATED `search_city_knowledge` RPC (reads only `business_id IS NULL`); `searchCityKnowledge` calls it instead of routing through the shared RPC. **Blast radius:** `search_knowledge_base` (business recommendations, every city) left byte-for-byte unchanged — verified live via `pg_proc`. `searchBusinessKnowledge` untouched. If the RPC is absent the call fails soft (empty city results = prior behaviour). **DB:** must run the `search_city_knowledge` SQL in each environment (file in repo is NOT proof it's applied — verify with `pg_proc`). |
| 0.35 Franchise Email Config Self-Heal | `app/api/admin/setup/route.ts`, `app/api/hq/franchises/route.ts` + data backfill | Low | **Bug:** new cities had `resend_from_email = null` — HQ create-franchise never set it, and the setup form's "From Email" field is read-only/display-only (no `onChange`, never persisted). This tripped the `resend_api_key && resend_from_email` gate → ALL franchise email blocked (claims, verification codes) for e.g. Kefalonia. **Fix:** setup route now ALWAYS derives `resend_from_email = no-reply@{city}.qwikker.com` on save (self-heals on next save, can't be blanked); HQ creation sets `resend_from_email` + `resend_from_name` at creation (insert + coming-soon update). Data backfill ran for all existing rows. **Blast radius:** the stored value is only a "configured" flag + reply-to/mailto display — actual send/reply-to addresses are auto-derived from city in `send-franchise-email.ts`. Setup route now writes this field unconditionally on save (was conditional) — intended. |
| 0.38 CRM Google Maps Link | `components/admin/comprehensive-business-crm-card.tsx` | Low | Added "View on Google Maps" in three spots: collapsed card header quick-actions, expanded Business Control Panel action bar (next to Sync), and the Address value in Contact Information (now a clickable link). New `getGoogleMapsUrl()` helper builds the best link: exact place via `google_place_id` → coordinates → name+address search. Existing rating-panel maps link upgraded to the same helper. Purely additive UI; no data or behaviour changes. |
| 0.37 Address Autocomplete Tenant-Aware | `components/ui/google-address-autocomplete.tsx` | Low | **Bug:** the manual street-address field (onboarding) hardcoded `country: 'gb'` AND used the global `NEXT_PUBLIC_GOOGLE_PLACES_API_KEY` env var — non-UK businesses got UK-only address suggestions, and all lookups billed one shared Google account instead of each franchise's own key. **Fix:** now fetches `/api/tenant/config` for the franchise's public key + country + center (same pattern as `google-places-autocomplete-v2`); loads Maps with the franchise key; only restricts `componentRestrictions` when a country is configured; biases suggestions to the city center via a `Circle.getBounds()`. **Blast radius:** single consumer (`simplified-onboarding-form.tsx` address field). Failure mode unchanged — falls back to a plain text input if config/key missing. Removes dependency on the global env var (correctness + billing win). Dead V1 `google-places-autocomplete.tsx` left untouched (unimported). |
| 0.36 Google Places Country Scoping | `app/api/tenant/config/route.ts`, `components/ui/google-places-autocomplete-v2.tsx`, `app/api/hq/franchises/route.ts` + data backfill | Low | **Bug:** `google_places_country` defaulted to `'gb'` in two places (tenant config + autocomplete) and was stored wrong (`gb`) for non-UK cities → business-verify autocomplete (manual listing + `/onboarding`) showed UK-only results (e.g. Kefalonia). `componentRestrictions.country` is a HARD filter, so location bias couldn't compensate. **Fix:** tenant config derives country from `google_places_country` → `country_code`, returns `null` (no restriction) if neither set — never hardcodes `gb`; autocomplete only applies `componentRestrictions` when a country is present; HQ creation sets `google_places_country = lower(country_code)` (insert + coming-soon update). Data backfill: `google_places_country = lower(country_code)`. **Blast radius:** `/api/tenant/config` `country` field is consumed ONLY by `google-places-autocomplete-v2` (Atlas consumers read no `country`; server-side import reads `franchise_crm_configs` directly). Component used ONLY by onboarding + dashboard verify. **Known NOT fixed (follow-up):** `GoogleAddressAutocomplete` (manual address field) still hardcodes `country:'gb'` + uses global env key — needs tenant-awareness. **Note:** cities with `has_center = false` (no lat/lng) are country-restricted but not location-biased until a center is set (Brighton = tutorial test city, intentionally unconfigured). |
