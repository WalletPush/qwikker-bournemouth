...
## 🐛 AI chat dish retrieval — PARTIAL FIX (Aug 8, 2026)

**Was:** dish queries / “what about the best burgers” could score Mel’s menu at 0 and surface junk (e.g. scooter rental).  
**Shipped on main:** `scoreMenuPreviewMatch` stopwords (`what`/`which`/…) + plural stem; food sanity filter excludes scooter/rental/hire. Roadmap `ai-chat-menu-item-retrieval` — **partial** (cold fuzzy business-name lookup still open).
...
- **⭐⭐ VERY IMPORTANT TODO (Jul 14) — Living Pass (iOS 27 Poster Generic) + Offer Redemption model. FULLY SCOPED IN CHAT, needs writing up + building.** Founder is actively pursuing this (has iOS 27 dev access; **his dad owns WalletPush** → platform features are requestable, not blockers). **Decisions locked:** (1) **Redemption = "just show the pass"** — ZERO friction, no staff scanner/PIN/POS (none exist yet). Anti-abuse = a **short self-expiry (drop 12h → 1h)** + a live ticking countdown (screenshots freeze = the "is it genuine" tell). Clear copy at Add-to-Wallet: *"active for 1 hour once added — only add when you're at the venue."* (2) **Single vs multiple is business-set** (`offer_claim_amount`) and must be **enforced server-side** — TODAY it's **localStorage-only theatre** (SQL proved a single-use offer claimed 3×; `claimOffer` inserts with no dedupe; `status='redeemed'` never written; `wallet_added` never lands). Rule: **single-use = can be ACTIVATED (added to wallet) once, ever**; merely saving to "My Claimed" doesn't burn it; multiple = re-activatable until offer end date. Enforce via `user_offer_claims` (table+writers exist: `lib/actions/offer-claim-actions.ts`, `hasUserClaimedOffer()` unused). (3) **Self-expire + pass-reset must be BUILT** (currently the "12h" is just a label, nothing resets — this is why Kefalonia looked stuck); mirror the loyalty pattern `app/api/loyalty/redemption/reset-pass/route.ts`. **BUG (Kefalonia):** claiming reset `AI_Url`/`Offers_Url` to template defaults "Sample AI_Url"/"Sample Offers_Url" — root cause = WalletPush **re-seeds unspecified fields from template defaults on partial field-PUT** (our `update-main-pass` only PUTs `Current_Offer`+`Last_Message`); push also came from **"SeahorseOne"** not Qwikker (template sender name). **Fixes:** (our side) make `update-main-pass` **self-heal** — re-assert `AI_Url`/`Offers_Url` on every write; (WalletPush side) stop clobbering unspecified fields + set per-city sender name. **iOS 27 capabilities (verified vs Apple docs + WWDC26 §209):** Poster Generic pass face = **background image + logo + fields + barcode** (dynamic bg per-member via `$${Background_Image}` placeholder / SDK admin API = "pass becomes the offer"); **Featured Actions** = up to **2** tiles, `{ id, type(enum), icon(SF Symbol), title/string, url }` — Apple DOES allow custom icon+label (WWDC video simplified to id/type/url; written docs confirm custom), but **WalletPush today only makes the URL dynamic per-member** (tile type/label fixed at template) — need Dad to expose the full Action object per-member. Redemption needs NO tile (show-it model) → 2 fixed tiles = **View Offers + Chat with AI (Concierge)**. **Stamp Card (iOS 27, WalletPush beta):** server-side composites the stamp grid from `${Points}` — kills per-count image pre-gen; loyalty is already **one pass per business** (`loyalty_programs` has own template/api key) so single `${Points}` maps 1:1, no conflict; drive from our loyalty balance via API. Need **12-icon earned/unearned artwork** (`stamp,bean,scissors,chilli,burger,cocktail,pizza,star,heart,cake,dumbbell,paw`) — automate via Qwikker-hosted URLs or (best) build natively into WalletPush. **Architecture = pass STATE MACHINE** (one background at a time; priority: active offer > time-of-day ambient > default) driven by a backend **resolver + scheduler**, configured/monitored via a new **admin "pass control room"** (design default pass, time-of-day rules, per-state preview iOS27 + iOS26 fallback, per-member inspector/force-refresh/self-heal). **Rollout is SAFE via dual keys** (`posterGeneric` + `generic`): iOS 26 ignores new stuff → ship early, progressively enhances. **Test plan:** iOS 27 dev beta on a SPARE device + Dad enables beta features on a Qwikker test template; matrix = iOS27 poster / iOS26 fallback / Google Wallet mirror; passes testable privately (signed .pkpass link, no App Store needed). **TODO deliverables:** write `LIVING_PASS_SPEC.md` (Qwikker architecture + redemption model) + `WALLETPUSH_REQUIREMENTS.md` (the Dad/platform build list: full dynamic Featured Actions, per-member background API, stamp `${Points}` API, fix field re-seeding, fix sender name, SDK admin API docs/keys). **NOT BUILT — decisions locked, spec+build pending.**
