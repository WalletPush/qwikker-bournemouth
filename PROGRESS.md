...
| 0.38 CRM Google Maps Link | `components/admin/comprehensive-business-crm-card.tsx` | Low | Added a "View on Google Maps" quick-action in the business CRM card header (always visible). New `getGoogleMapsUrl()` helper builds the best link: exact place via `google_place_id` → coordinates → name+address search. Existing rating-panel maps link upgraded to the same helper (was a name+town search). Purely additive UI; no data or behaviour changes. |
