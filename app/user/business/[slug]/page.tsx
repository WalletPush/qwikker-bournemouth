...
export default async function BusinessDetailPage({ params, searchParams }: BusinessDetailPageProps) {
  ...
  const realBusinesses = (activeBusinesses || []).map(business => {
    ...
    return {
      ...
      vibes: null as Awaited<ReturnType<typeof getBusinessVibeStats>>, // filled for viewed listing below
  ...
  // 💚 Vibes only for the listing being viewed (was N queries for the whole city)
  if (viewedBusiness) {
    viewedBusiness.vibes = await getBusinessVibeStats(viewedBusiness.id)
  }
